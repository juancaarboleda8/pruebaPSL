...
public class ImpresorLCD{
    ...
    private void adicionarDigito(int numero) {
        ...
        while (iterator.hasNext()) {
            segmentoSegunCaso(iterator.next());
        }
    }
    ...
    private void procesar(String comando, int espacioDig) {
        ...
        parametros = comando.split(",");
         ...
         validarCantidadParametros(parametros.length, comando);
   ...
   private boolean isNumeric(String cadena) {
    ...
    public int validarNumeroYRango(String valor_ingresado){
        if(isNumeric(valor_ingresado)){
            int espacioDig = Integer.parseInt(valor_ingresado);
            return validarSuRango(espacioDig);
        }else{
            throw new IllegalArgumentException("Cadena " + valor_ingresado
                            + " no es un entero");
        }
    }
    
   private int validarSuRango(int valor_rango){
        if(valor_rango <0 || valor_rango >5){
            if(valor_rango <1 || valor_rango >10){
                throw new IllegalArgumentException("El parametro size ["+valor_rango
   ...
   public void mostrarResultado(List numerosLCD,int espacioDig){
       Iterator<String> iterator = numerosLCD.iterator();
       while(iterator.hasNext()){
           try {
   ...
   private void validarCantidadParametros(int numeroDeComas,String comando){
       if(numeroDeComas!=2){
           throw new IllegalArgumentException("Cadena " + comando
                    + " contiene mas caracter o menos caracteres"); 
       }   
      
   }
   
   private void segmentoSegunCaso(int segmento){
       if(segmento <= 4){
           switch(segmento){
           case 1:
